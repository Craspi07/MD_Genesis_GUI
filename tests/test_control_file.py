...
def test_render_aicg2p_contains_sections():
    text = render_control_file(_config(), ModelType.AICG2P)
    for section in ["[INPUT]", "[OUTPUT]", "[ENERGY]", "[DYNAMICS]", "[CONSTRAINTS]", "[ENSEMBLE]", "[BOUNDARY]"]:
        assert section in text
    assert "forcefield       = AICG2+" in text
...
def test_timestep_converted_fs_to_ps():
    text = render_control_file(_config(timestep_fs=15.0), ModelType.AICG2P)
...
def test_write_control_file_creates_file(tmp_path: Path):
    path = write_control_file(_config(), ModelType.AICG2P, str(tmp_path), filename="run.inp")
    assert path.exists()
    assert path.read_text() == render_control_file(_config(), ModelType.AICG2P)


def test_write_control_file_preserves_manual_edits_unless_forced(tmp_path: Path):
    path = write_control_file(_config(), ModelType.AICG2P, str(tmp_path))
    path.write_text("; hand-edited by user\n")

    same_path = write_control_file(_config(), ModelType.AICG2P, str(tmp_path))
    assert same_path.read_text() == "; hand-edited by user\n"

    overwritten_path = write_control_file(_config(), ModelType.AICG2P, str(tmp_path), force=True)
